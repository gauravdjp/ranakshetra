...
function EmptyState({ label }: { label: string }) {
  return (
    <div className="text-center py-16">
      ...
      <p className="font-[Rajdhani,sans-serif] text-white/15 text-sm mt-2 tracking-wide">Check back soon!</p>
    </div>
  );
}
...
/* ─────────────────────────────────────────────────────────────
   Tournaments Section – same as player page
   ───────────────────────────────────────────────────────────── */
function TournamentsSection() {
  ...
  useEffect(() => {
    ...
    const checkAll = async () => {
      ...
      finally { setCheckingJoined(false); }
    };
    checkAll();
  }, [tournaments]);
                ...
                <div className="flex-shrink-0">
                  {checkingJoined ? (
                    <button disabled className="join-btn px-4 py-2 text-[0.6rem] uppercase font-bold tracking-[0.18em] font-[Rajdhani,sans-serif] whitespace-nowrap opacity-40 bg-[rgba(139,92,246,0.15)] border border-[rgba(139,92,246,0.4)] text-[#a78bfa]"
                      style={{ clipPath: "polygon(4px 0%, 100% 0%, calc(100% - 4px) 100%, 0% 100%)" }}>...</button>
                  ) : joined.includes(tid) ? (
                    <button onClick={() => router.push(`/tournaments/${tid}`)}
                      ...
                      style={{ clipPath: "polygon(4px 0%, 100% 0%, calc(100% - 4px) 100%, 0% 100%)" }}>Check Out →</button>
                  ) : (
                    <button onClick={() => handleJoin(tid)}
...
/* ─────────────────────────────────────────────────────────────
   CLUB APPLY MODAL – same as player page
   ───────────────────────────────────────────────────────────── */
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center px-4" onClick={onClose}>
      ...
      <div className="modal-panel relative w-full max-w-[460px] bg-[#09091a] border border-[rgba(139,92,246,0.35)] p-8"
        ...
        <button className="apply-btn w-full py-3 font-[Rajdhani,sans-serif] font-bold text-[0.88rem] tracking-[0.2em] uppercase text-white mb-3 border-none cursor-pointer"
          onClick={onClose}>Notify Me When Open</button>
        <button onClick={onClose}
...
/* ─────────────────────────────────────────────────────────────
   Clubs Section – same as player page
   ───────────────────────────────────────────────────────────── */
        ...
        {clubs.map((club, i) => {
          ...
          return (
            <div key={club._id ?? i}
              ...
              {/* Apply button */}
              <button onClick={() => setApplyClub(club)}
                ...
                style={{ clipPath: "polygon(4px 0%, 100% 0%, calc(100% - 4px) 100%, 0% 100%)" }}>Apply →</button>
            </div>
          );
        })}
      </div>
...
/* ─────────────────────────────────────────────────────────────
   Arenas Section – same as player page
   ───────────────────────────────────────────────────────────── */
          ...
          <div key={arena._id ?? i}
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="font-[Rajdhani,sans-serif] text-[0.58rem] text-white/18 mt-0.5">
                {arena.arena_location}{arena.arena_city ? ` · ${arena.arena_city}` : ""}{arena.organizer_name ? ` · by ${arena.organizer_name}` : ""}
              </p>
            </div>
            {/* Stats */}
            <div className="hidden md:flex items-center gap-5 shrink-0">
...
/* ─────────────────────────────────────────────────────────────
   MAIN LAYOUT SHELL
   ───────────────────────────────────────────────────────────── */
export default function OrganiserMainPage() {
  ...
  return (
    <>
      <style>{`
        ...
        @keyframes contentIn { from { opacity: 0; transform: translateX(10px); } to   { opacity: 1; transform: translateX(0); } }
        @keyframes livePulse { 0%, 100% { opacity: 1; box-shadow: 0 0 6px #8b5cf6; } 50%       { opacity: 0.4; box-shadow: 0 0 2px #8b5cf6; } }
        @keyframes spinPulse { 0%   { transform: rotate(0deg);   opacity: 0.4; } 50%  { opacity: 1; } 100% { transform: rotate(360deg); opacity: 0.4; } }
        ...
        .spin-loader { width: 28px; height: 28px; border: 2px solid rgba(139,92,246,0.15); border-top-color: #8b5cf6; border-radius: 50%; animation: spinPulse 1s linear infinite; }
        .nav-link { font-family: 'Rajdhani', sans-serif; font-size: 0.72rem; letter-spacing: 0.28em; font-weight: 500; text-transform: uppercase; padding: 10px 16px; border-left: 2px solid transparent; color: rgba(255,255,255,0.28); cursor: pointer; transition: all 0.2s ease; white-space: nowrap; display: block; width: 100%; text-align: left; background: none; border-right: none; border-top: none; border-bottom: none; }
        .nav-link:hover { color: rgba(255,255,255,0.65); border-left-color: rgba(139,92,246,0.35); background: rgba(139,92,246,0.04); }
        .nav-link.active { color: #a78bfa; border-left-color: #8b5cf6; background: rgba(139,92,246,0.08); }
        .avatar-btn { width: 38px; height: 38px; border-radius: 50%; border: 1.5px solid rgba(139,92,246,0.45); background: rgba(139,92,246,0.1); display: flex; align-items: center; justify-content: center; cursor: pointer; transition: all 0.2s ease; }
        .avatar-btn:hover { border-color: rgba(139,92,246,0.8); background: rgba(139,92,246,0.18); box-shadow: 0 0 14px rgba(139,92,246,0.3); }
        .avatar-btn.open { border-color: rgba(139,92,246,0.9); box-shadow: 0 0 20px rgba(139,92,246,0.35); }
        .dropdown { position: absolute; top: calc(100% + 10px); right: 0; width: 180px; background: #0a0a1c; border: 1px solid rgba(139,92,246,0.3); z-index: 100; overflow: hidden; box-shadow: 0 20px 60px rgba(0,0,0,0.6), 0 0 30px rgba(139,92,246,0.1); }
      `}</style>
      {/* Header */}
      <div className="fixed inset-0 bg-[#0a0a1c]" />
      <div className="fixed inset-0 overflow-y-auto">
        <div className="flex min-h-screen">
          {/* Sidebar */}
          <nav className="w-64 bg-[#070718] border-r border-[rgba(139,92,246,0.12)] p-6 flex flex-col">
            <div className="flex items-center mb-8">
              <div className="font-[Cinzel,serif] text-2xl font-bold text-[#a78bfa]">RANAKSHETRA</div>
            </div>
            <div className="flex-1 space-y-1">
              <div className="nav-link active" onClick={() => setActiveSection('dashboard')}>Dashboard</div>
              {NAV_LINKS.map(link => (
                <div key={link.key} className={`nav-link ${activeSection === link.key ? 'active' : ''}`} onClick={() => setActiveSection(link.key as NavSection)}>{link.label}</div>
              ))}
            </div>
            <div className="mt-8">
              <div className="flex items-center gap-3">
                <div className="avatar-btn" ref={avatarRef} onClick={() => setAvatarOpen(!avatarOpen)}>{initials}</div>
                {avatarOpen && (
                  <div className="dropdown">
                    <div className="p-4">
                      <p className="text-white/70">{displayName}</p>
                      <button className="mt-2 text-sm text-[#8b5cf6]" onClick={() => signOut()}>Sign Out</button>
                    </div>
                  </div>
                )}
              </div>
