...
export default function Bracket({ id, isBye = false, team1 = "TeamAlpha", team2 = "TeamOmega" }: BracketProps) {
  ...
  const handleResult = (team: 1 | 2, result: "W" | "L") => {
    if (locked || isBye) return;
    if (team === 1) {
      setTeam1Result(result);
      setTeam2Result(result === "W" ? "L" : "W");
    } else {
      setTeam2Result(result);
      setTeam1Result(result === "W" ? "L" : "W");
    }
    setLocked(true);
  };

  // ── BYE early return — single card, no opponent
  if (isBye) {
    return (
      <div className="flex items-center gap-3 mt-10 ml-10">
        ...
        {/* Connector — id here for arrow anchoring */}
